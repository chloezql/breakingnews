...
export function useWebSocket({ onMessage, onConnect, onDisconnect }: UseWebSocketOptions = {}) {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const wsUrl = process.env.REACT_APP_WS_URL || 'wss://breaking-news-ws-server-production.up.railway.app';
  ...
  return { sendMessage, disconnect, reconnect, isConnected: !!wsRef.current };
} 